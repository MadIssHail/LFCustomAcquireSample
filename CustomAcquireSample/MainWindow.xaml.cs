...
namespace Custom_Acquire_Sample_Automation_
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void UpdateListBox()
        {
            ...
            foreach (var camera in experiment_.AvailableDevices)  //Will only show if camera is in LF Available Devices when the application starts.
                if (experiment_.AvailableDevices.Count > 0 &&     //If the hidden LF instance is used to add cameras, they won't show up in the Available Devices list
        ...
        {            
            ...
            foreach (var camera in experiment_.AvailableDevices)                                //If camera is in Available Devices 
                if (experiment_.AvailableDevices.Count > 0 &&
                    camera.Type == DeviceType.Camera)
                {
                    experiment_ = new OpenLightField(this, visibleLightField:true).Experiment;  //Open an instance of LightField for every camera in Available Devices
                    ...
                    string cameraName =
                        string.Format("Model: {0}, SN: {1}",
                        camera.Model, camera.SerialNumber);                                     //For list in UI      
